if (!init)
{
    ...
    {
        var mark = scr_marker_fromasset(findsprite(1998, "CUT_ASSETS", shrub_colors[i]));
        with (mark)
        {
            if (image_blend == 255)
            {
    ...
    with (126)
    {
        ...
        add_sprite_offset(6052, 1, 0);
        add_sprite_offset(7574, 1, 0);
        add_sprite_offset(4099, 0, -4);
        substitute_sprite(4099, 6364);
        add_sprite_offset(2940, 0, -4);
        substitute_sprite(2940, 8017);
        add_sprite_offset(8378, 0, -4);
        substitute_sprite(8378, 2209);
        add_sprite_offset(2579, 0, -4);
        substitute_sprite(2579, 5884);
    }
}
if (con == 0)
{
    if (global.tempflag[tempflag_index] < 1 && false)
    {
        con = 1;
        alarm[0] = 1;
        global.interact = 1;
...
if (con == 1.1)
{
    ...
    ora = actor_count + 1;
    ora_actor = instance_create(orapos.x, orapos.y, 1559);
    scr_actor_setup(ora, ora_actor, "orange");
    ora_actor.sprite_index = 8247;
    ora_actor.x = 320;
    ora_actor.visible = true;
    pk = actor_count + 2;
    pk_actor = instance_create(380, pkpos.y - 200, 1559);
    scr_actor_setup(pk, pk_actor, "pink");
    pk_actor.sprite_index = 8378;
    c_sel(kr);
    ...
    c_customfunc(function()
    {
        ...
        su_actor.sprite_index = 1027;
        ra_actor.sprite_index = 1447;
    });
    c_wait(30);
    con = 3;
    c_actortokris();
...
if (con == 2)
{
    ...
    ora_actor = instance_create(orapos.x, orapos.y, 1559);
    scr_actor_setup(ora, ora_actor, "orange");
    ora_actor.sprite_index = 5678;
    ora_actor.x = 320;
    ora_actor.visible = true;
    pk = actor_count + 2;
    pk_actor = instance_create(pkpos.x, pinkface.y - 200, 1559);
    scr_actor_setup(pk, pk_actor, "pink");
    pk_actor.sprite_index = 3733;
    pk_actor.visible = false;
    su_actor.mask_index = 587;
    c_sel(kr);
    c_setxy(krpos.x - 250, krpos.y);
    c_sel(su);
    c_setxy(supos.x - 250, supos.y);
    ...
    c_msgcmd("addxy", 0, -2);
    c_spritenext(8232, 0, 0.25);
    c_talk_wait();
    c_sel_sprite(pk, 3963);
    c_imageindex(0);
    c_wait(4);
    c_animate(0, 2, 0.1);
    c_speaker("pink");
    ...
    c_sel(su);
    c_spritenext(7693, 0, 0);
    c_sel(ra);
    c_spritenext(6865, 0, 0);
    c_facenext("susie", "K");
...
if (con == 3 && customcon == 1)
{
    ...
    if (!battlesetup && i_ex(1412))
    {
        battlesetup = true;
    }
    timer++;
    if (timer > 45 && !i_ex(1412))
    {
        pk_actor.visible = true;
        timer = 0;
        con = 4;
        scr_maincharacters_actors();
        pk_actor.sprite_index = 6792;
...
if (con == 5)
{
    ...
    c_talk_wait();
    c_wait(10);
    c_sel_sprite(su, 3560);
    c_sel_sprite(ra, 4130);
    c_wait(30);
    c_speaker("ralsei");
    ...
    c_sel_spriteimage(pk, 2579, 0, 0.25);
    c_msgstay(3);
    c_speaker("pink");
    ...
    c_sel_spritenext(pk, 2579, 0, 0.25);
    c_msgvar_instance(pinkface, "sweat", false);
    c_talk_wait();
    c_autowalk(true);
    c_walk("r", 5, 7);
    c_wait(7);
    c_spriteimage(3163, 0, 0);
    c_autowalk(false);
    c_var_lerp_to_instance(pk_actor, "image_index", 2, 6);
    c_wait(6);
    c_snd_play(504);
    ...
    c_spritenext(4979, 0, 0);
    c_talk_wait();
    c_autowalk(true);
    c_walk("r", 5, 7);
    c_wait(7);
    c_spriteimage(8427, 0, 0);
    c_addxy(-1, 0);
    c_autowalk(false);
    c_wait(6);
    c_wait(15);
    c_imageindex(1);
    c_snd_play(479);
    kissheart = scr_dark_marker(160, 121, 3065);
    ...
    c_spriteimage(3735, 0, 0);
    c_var_lerp_to_instance(pk_actor, "x", 65, 20, 3, "out");
    c_shakeobj();
    c_wait(24);
    c_autowalk(false);
    c_spriteimage(2579, 0, 0.25);
    c_speaker("pink");
