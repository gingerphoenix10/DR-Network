...
if (color == 1)
{
    ...
    if (z_hold >= 15)
    {
        ...
        for (i = 0; i < 4; i += 1)
        {
            ...
            draw_sprite_ext(3492, 0, (x + 9) - xx, (y + 10) - yy, 4 - ((z_charge * 2) / 35), 4 - ((z_charge * 2) / 35), 0, 16777215, z_charge / 5);
        }
    }
}
if (color == 2)
{
    if (dash_act == 1 && dash_t > 45)
    {
        ...
        for (i = 0; i < 30; i++)
        {
            draw_sprite_ext(7326, 0, tx + x_offset, ty, image_xscale, image_yscale, 0, 4235519, 0.025);
    ...
    var _box = -4;
    with (collision_point(x + 5, y + 5, 374, false, true))
    {
        _box = id;
    }
    if (dash_charge >= 6)
    {
        draw_set_alpha(0.5);
        var al = 2;
        draw_set_colour(merge_colour(4235519, 65535, clamp(0.5 + sin(i + (current_time / 55)), 0, 1)));
        for (i = 0; i < (ceil((dash_charge / dash_max) * 120) / 10); i++)
        {
            draw_sprite_ext(5522, 0, x + (i * 16) + aim_x + 16 + x_offset, y + 10, 1, 1, 0, draw_get_colour(), 1);
            al -= 0.1;
        }
        draw_set_alpha(1);
    }
    ...
    if (dash_h > 0)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            if (i > 3)
            {
                col = merge_colour(255, 4235519, (i - 3) / 3);
            }
            else
            {
                col = merge_colour(0, 255, i / 3);
            }
            tailx[i] -= 2 * (6 - i);
            taila[i] -= 0.5;
            draw_sprite_ext(7326, 1, x + _w + tailx[i] + x_offset, y + _h, image_xscale * xs, image_yscale * ys, 0, col, taila[i]);
        }
    ...
    {
        draw_sprite_ext(5406, 3, x + _w + x_offset, y + _h, image_xscale * xs, image_yscale * ys, 0, merge_colour(255, 65535, clamp(0.5 + (sin(current_time / 50) * 0.5), 0, 1)), 1);
    }
    else if (dash_h == 0 && dash_delay <= 0)
    {
        draw_sprite_ext(5406, 3, x + _w + x_offset, y + _h, image_xscale * xs, image_yscale * ys, 0, 255, 1);
    }
}
else
{
