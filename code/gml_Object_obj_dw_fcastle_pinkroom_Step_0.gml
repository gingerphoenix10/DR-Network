if (!init)
{
    ...
    {
        var mark = scr_marker_fromasset(findsprite(1998, "NPCs", shrub_colors[i]));
        with (mark)
        {
    ...
    {
        var saveloc = findspriteinfo(1888);
        instance_create(saveloc.x, saveloc.y, 1236).depth -= 120;
    }
    else
    {
        cutscenecon = 0.1;
    ...
    if (global.flag[PINK_FLAG] >= 3)
    {
        ...
        {
            with (instance_create(gm.x, gm.y, 1665))
            {
                image_xscale = 2;
                image_yscale = 20;
            }
...
else if (cutscenecon == 1)
{
    ...
    var orapos = findspriteinfo(7965, "CUT_ASSETS");
    var orapos2 = findspriteinfo(8247, "CUT_ASSETS");
    ora = actor_count + 1;
    ora_actor = instance_create(orapos.x, orapos.y, 1559);
    scr_actor_setup(ora, ora_actor, "orange");
    ora_actor.sprite_index = 7965;
    ...
    c_sel(ora);
    c_sprite(5678);
    var orapos3 = findspriteinfo(1617, undefined, 16711680);
...
if (cutscenecon == 2 && !instance_exists(1570))
{
    global.interact = 0;
    cutscenecon = 0;
