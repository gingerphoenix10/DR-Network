...
if (resize_mode)
{
    draw_surface_ext(surf_resize, 0, 0, 2, 2, 0, 16777215, 1);
}
else
{
    if (!skip_main_characters)
...
if (!skip_main_characters && alwaysfalse)
{
    ...
    cast_shadow_self(obj_charafake);
}
...
if (global.darkzone)
{
    ...
    if (!skip_main_characters && alwaysfalse)
    {
        ...
        cast_highlight_cutout(obj_charafake);
...
draw_surface_ext(surf_dyna_shadows, cx, cy, 1, 1, 0, 16777215, 1 * _shadow_alpha);
