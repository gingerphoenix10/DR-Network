...
draw_reflection = function()
{
    if (instance_exists(owner))
    {
        ...
        else if (owner.object_index == 1389 && !global.showcompanions)
        {
            exit;
        }
        if (owner.visible)
...
reflectobjs = [1198, 1389, obj_charafake];
for (var i = 0; i < array_length(reflectobjs); i++)
{
    with (reflectobjs[i])
    {
        ...
        myreflection = instance_create_layer(x, y, layer_get_id("REFLECTSURF"), 1496);
        with (myreflection)
        {
            mindepth = depth;
            owner = other.id;
...
make_reflection = function(arg0)
{
    ...
    else if (arg0.object_index == 1389 && !global.showcompanions)
    {
        exit;
    }
    var _reflectionmarker = instance_create_layer(x, y, layer_get_id("REFLECTSURF"), 1496);
...
{
    var _a = scr_marker_fromasset(findsprite(6405, "NPCs", 16777215));
    with (_a)
    {
        scr_depth(self, -40);
        image_blend = 16777215;
    }
    _a = scr_marker_fromasset(findsprite(6405, "NPCs", 255));
    with (_a)
    {
        scr_depth(self, -40);
        image_blend = 16777215;
    }
    _a = scr_marker_fromasset(findsprite(6405, "NPCs", 65280));
    with (_a)
    {
        scr_depth(self, -40);
        image_blend = 16777215;
    ...
    npc_green = scr_makenpc_fromasset(findsprite(7883, "NPCs"));
    ...
    npc_orange = scr_makenpc_fromasset(findsprite(7216, "NPCs"));
