...
{
    grassinit = true;
    use_foreground_marker = instance_exists(504);
    ...
    if (instance_exists(1063) || instance_exists(1247) || instance_exists(998))
    {
        dashspeed_modified *= 2.5;
    }
    if (instance_exists(689) || instance_exists(250))
    {
    ...
    if (!instance_exists(651))
    {
        instance_create(0, 0, 651);
    }
    if (instance_exists(28) && !instance_exists(937))
    {
        var _petalwing = instance_create(x, y, 937);
        _petalwing.in_use = 1;
...
if (dash_transition_con > 0 && !targetmode)
{
    ...
    if (dash_transition_con == 1)
    {
        ...
        if (dash_transition_timer == 1)
        {
            ...
            collision_rectangle_list(x + (40 * image_xscale), y, x + (41 * image_xscale), y + 200, 706, false, true, li, false);
        ...
        if (dash_transition_timer == 17)
        {
            ...
            global.interact = 0;
            vfx = instance_create(x, y, 825);
            vfx.owner = id;
            vfx.xspeed = -15 * image_xscale;
            dashing = true;
            attacking = false;
            hspeed = dashdir * 12;
            audio_stop_sound(400);
            snd_play(400);
            windloop_sound = snd_loop(647);
        }
    }
...
if (state == 1)
{
    ...
    if (instance_exists(_FollowObject))
    {
        ...
        if (grounded)
        {
            if (hspeed > 0 && !collision_line(x, y, x, bbox_bottom + 10, 706, 0, 0) && !collision_line(x, y, x, bbox_bottom + 10, obj_plat_player.ground, 0, 0))
            {
                press_jump = true;
            }
            if (hspeed < 0 && !collision_line(x, y, x, bbox_bottom + 10, 706, 0, 0) && !collision_line(x, y, x, bbox_bottom + 10, obj_plat_player.ground, 0, 0))
...
if (in_targetmode)
{
    if (!last_targetmode)
    {
        ...
        if (instance_exists(1390) && obj_darkcontroller.charcon == 1)
        {
            pad_bottom += 40;
        }
        collision_rectangle_list(_cx + pad, _cy + pad, (_cx + camerawidth()) - pad, (_cy + cameraheight()) - pad_bottom, 563, false, false, act_targets_list, false);
        targetmode = true;
        targetindex = -1;
        blue_outline = true;
        heart.depth = 1400;
...
if (!hurt)
{
    ...
    if (dashing && instance_place(x + (hspeed * 1.5), y, 1424))
    {
        attacking = 0;
        dashing = 0;
        jumping = 3;
        ...
        vspeed = -14;
        hspeed = -hspeed * 0.25;
        snd_play_pitch(454, 1.2);
        snd_play(371);
        with (instance_create(x + (20 * image_xscale), y, 1658))
        {
            sprite_index = 7342;
            alarm[0] = 2;
    ...
    var _gate = instance_place(x, y, 491);
...
if (key_left && instance_place(x - 8 - abs(hspeed), y, 1424))
{
    _dont_accel = true;
}
if (key_right && instance_place(x + 8 + abs(hspeed), y, 1424))
{
    _dont_accel = true;
}
if (climbing)
...
{
    var inc = 1;
    var meet = place_meeting(x, y - 8, 1424);
    if (meet)
    {
        inc = 0.5;
        if (jumpsquat == 0)
...
if (fallen_in_pit == 0)
{
    var _checkpoint = instance_place(x, y, 1283);
    if (_checkpoint != -4)
    {
...
if (fallen_in_pit == 0 && !instance_exists(1570) && y > (_cameray + _cameraheight + 120 + (sprite_height * 0.5)))
{
    if (dogjoke)
    {
        dogjoke = false;
...
if (fallen_in_pit > 0)
{
    ...
    else if (respawn_leap_style == 2)
    {
        ...
        if (fallen_in_pit == 3 && vspeed > -5 && !instance_place(x, y, 1424))
        {
            fallen_in_pit = 0;
            physics = true;
        }
...
if (jumphovering == true)
{
    ...
    if (!jumphover_shiftmode)
    {
        ...
        if (_hinput != 0 || _vinput != 0)
        {
            ...
            if (!instance_place(x + (moveX * 1.5), y, 1424))
            {
                x += moveX;
                _pull = true;
            }
            if (!instance_place(x, y + (moveY * 1.5), 1424))
            {
                y += moveY;
                _pull = true;
            }
...
if (jumphover_shiftmode)
{
    ...
    if (jumphovering && heart_mode == 1)
    {
        ...
        {
            if (!instance_place(x - abs(_speed), y, 1424))
            {
                x -= _speed;
            }
            _pull = true;
        ...
        {
            if (!instance_place(x, y - abs(_speed), 1424))
            {
                y -= _speed;
            }
            _pull = true;
...
if (!animation_override)
{
    ...
    {
        if (((!key_left && !key_right) || (key_left && key_right)) && !runstop_anim && !land_anim && sprite_index != 8216 && sprite_index == 6631)
        {
            turn_anim = false;
            runstop_anim = true;
            sprite_index = 7227;
    ...
    if (key_left || key_right || sprite_index != 7227)
    {
        runstop_anim = false;
    }
    if (attacking || !grounded)
    ...
    if (sprite_index != 266 || key_left || key_right)
    {
        land_anim = 0;
    }
    ...
    if (attacking)
    {
        if (grounded)
        {
            if (sprite_index == 5332 && image_index > 10)
            {
                land_anim = 1;
                sprite_index = 266;
                image_index = 0;
...
if (_landingsound && sprite_index == 266)
{
    snd_play_pitch(314, 1.2);
}
