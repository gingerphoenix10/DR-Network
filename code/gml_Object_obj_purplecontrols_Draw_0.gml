...
if (!can_move)
{
    _prpl_light = merge_color(_prpl_light, 0, 0.5);
    _prpl_dark = merge_color(_prpl_dark, 0, 0.5);
    _prpl_darker = merge_color(_prpl_darker, 0, 0.5);
    _prpl_darkest = merge_color(_prpl_darkest, 0, 0.5);
}
switch (mode)
{
    case 1:
        ...
        do
        {
            ...
            do
            {
                draw_sprite_ext(4469, 1, _xx, _yy, 1, 1, 0, _prpl_darker, 1);
                _xx += 32;
            }
            until (_xx >= 142);
            _yy += 24;
        ...
        do
        {
            draw_sprite_ext(4469, 0, _xx, 0, 1, 1, 0, _prpl_darker, 1);
            _xx += 32;
        }
        until (_xx >= 142);
        gpu_set_blendmode(0);
        ...
        _xx = 70;
        _yy = 70;
        draw_surface_ext(surface_effect, _xx - 70, _yy - _arrows_height, 1, 1, 0, 16777215, 1);
        draw_surface_ext(surface_effect, (_xx - 70) + 142, (_yy - _arrows_height) + (_arrow_pokethrough * 2), 1, 1, 180, 16777215, 1);
        draw_surface_ext(surface_effect, (_xx - 70) + 142, _yy + _arrows_height + 2, 1, 1, 180, 16777215, 1);
        draw_surface_ext(surface_effect, _xx - 70, ((_yy + _arrows_height) - (_arrow_pokethrough * 2)) + 2, 1, 1, 0, 16777215, 1);
        ...
        with (1535)
        {
            draw_surface_ext(other.surface_box, x + lengthdir_x(49.497475 * image_xscale, 135 + image_angle), y + lengthdir_y(49.497475 * image_yscale, 135 + image_angle), image_xscale / 2, image_yscale / 2, image_angle, 16777215, 1);
        }
        with (1025)
        {
            draw_self();
    ...
    case 3:
        ...
        if (arrow_alpha > 0)
        {
            draw_sprite_ext(8306, 1, x + 57, y + 18 + abs(lengthdir_y(10, arrow_dir)), 1, 1, 270, 16777215, arrow_alpha);
            draw_sprite_ext(8306, 1, x + 57, y - 16 - abs(lengthdir_y(10, arrow_dir)), -1, 1, 270, 16777215, arrow_alpha / 3);
        }
        with (1025)
        {
            draw_self();
    ...
    case 8:
            ...
            repeat (4)
            {
                if (draw_connection[i])
                {
                    if (instance_exists(child[i]))
                    {
                        ...
                        if (pulse >= 0)
                        {
                            var _col = merge_color(255, _prpl_dark, 0.667);
                            ...
                            draw_vertex_color(x + 1.5 + lengthdir_x(_drawdist, _dir) + lengthdir_x(1, _dir + 270), y + 1.5 + lengthdir_y(_drawdist, _dir) + lengthdir_y(1, _dir + 270), _col, 1);
                            draw_primitive_end();
                            _col = merge_color(255, _prpl_dark, 0.667);
                            draw_primitive_begin(5);
                            draw_vertex_color(x + 1.5 + lengthdir_x(_drawdist, _dir) + lengthdir_x(1, _dir + 90), y + 1.5 + lengthdir_y(_drawdist, _dir) + lengthdir_y(1, _dir + 90), _col, 1);
                            draw_vertex_color(x + 1.5 + lengthdir_x(_drawdist, _dir) + lengthdir_x(1, _dir + 270), y + 1.5 + lengthdir_y(_drawdist, _dir) + lengthdir_y(1, _dir + 270), _col, 1);
        ...
        if ((life_time % 60) <= 30)
        {
            _glow_color = merge_color(lightpink, 16777215, (0.75 * (life_time % 60)) / 30);
        }
        else
        {
            _glow_color = merge_color(lightpink, 16777215, 0.75 - ((0.75 * ((life_time - 30) % 60)) / 30));
        }
        with (1333)
        {
            if (checkpoint < 1)
        ...
        var _bleed = 4;
        draw_surface_ext(surface_effect, camerax(), cameray() - _bleed, 1, 1, 0, 0, _alpha);
        draw_surface_ext(surface_effect, camerax(), cameray() + _bleed, 1, 1, 0, 0, _alpha);
        draw_surface_ext(surface_effect, camerax() - _bleed, cameray(), 1, 1, 0, 0, _alpha);
        draw_surface_ext(surface_effect, camerax() + _bleed, cameray(), 1, 1, 0, 0, _alpha);
        _alpha = 0.5 * _alpha_master;
        _bleed = 2;
        draw_surface_ext(surface_effect, camerax() - _bleed, cameray() - _bleed, 1, 1, 0, 0, _alpha);
        draw_surface_ext(surface_effect, camerax() - _bleed, cameray() + _bleed, 1, 1, 0, 0, _alpha);
        draw_surface_ext(surface_effect, camerax() + _bleed, cameray() - _bleed, 1, 1, 0, 0, _alpha);
        draw_surface_ext(surface_effect, camerax() + _bleed, cameray() + _bleed, 1, 1, 0, 0, _alpha);
        if (_alpha_master < 1)
        {
            draw_surface_ext(surface_effect, camerax(), cameray(), 1, 1, 0, 16777215, _alpha_master);
        }
        else
        {
