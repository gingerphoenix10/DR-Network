...
for (var j = 0; j < (areasize * areasize); j++)
{
    ...
    with (watersplashes[j])
    {
        ...
        if (place_meeting(x, y, obj_waterreflect))
            visible = false;
        ...
        for (var i = 0; i < amount1; i++)
        {
            ...
            var can_water = elem.get_waterable() && elem.object_index != obj_waterable_asset;
            ...
            if (can_water)
            {
                ...
                {
                    if (elem.object_index == obj_waterable_trigger)
                    {
                        with (elem)
                        {
...
if (is_watering_ralsei)
{
    ...
    if (ralsei_watering_timer == 150 && !instance_exists(flowermarker))
    {
        with (obj_caterpillarchara)
        {
            ...
            {
                facing[target] = 0;
                other.flowermarker = instance_create(x, y - 6 - (12 * global.flag[1311]), obj_marker);
                
                with (other.flowermarker)
                {
                    sprite_index = spr_ralsei_flower_only;
                    scr_darksize();
                    depth = other.depth - 1;
                    image_speed = 0;
                    scr_lerpvar("image_index", 0, 6, 12);
                    snd_play(snd_harp_strum, 1, 1.1);
                }
            }
        }
    }
